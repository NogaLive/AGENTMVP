...
class Settings(BaseSettings):
    ...
    model_id: str = Field(default="nvidia/nemotron-3-ultra-550b-a55b:free", alias="MODEL_ID")

    # Persistencia Vectorial (Supabase)
    supabase_url: str = Field(default="", alias="SUPABASE_URL")
    supabase_secret_key: Optional[str] = Field(default=None, alias="SUPABASE_SECRET_KEY")
    supabase_publishable_key: Optional[str] = Field(default=None, alias="SUPABASE_PUBLISHABLE_KEY")

    # Servidor FastMCP
    mcp_url: str = Field(default="http://127.0.0.1:8001/", alias="MCP_URL")

    # Autenticación y Seguridad
    jwt_secret: str = Field(default="sbs-plaft-super-secret-key-2026-production", alias="JWT_SECRET")
    jwt_algorithm: str = Field(default="HS256", alias="JWT_ALGORITHM")
    jwt_expiration_minutes: int = Field(default=10080, alias="JWT_EXPIRATION_MINUTES") # 7 días
    password_salt: str = Field(default="sbs-plaft-salt-security", alias="PASSWORD_SALT")
